...
typedef struct __DHDEV_CAMERA_CFG 
{
	...
	BYTE                bReserved_1;            //字节对齐
	WORD                wExternalSyncValue ;    // 外同步的相位设置 0~360
	char				bRev[58];				// 保留
} DHDEV_CAMERA_CFG;
...
typedef struct __DEV_ENCODER_INFO 
{
	...
	BYTE            byManuFactory;						//目标设备的生产厂商,0:Private(大华私有), 1: AEBell(美电)
...
typedef struct  
{
	char               szDevSerial[DH_INTERVIDEO_AMP_DEVICESERIAL];                  //(只读)前端设备序列号,即编码器的序列号	
	char               szDevName[DH_INTERVIDEO_AMP_DEVICENAME];                      //前端设备名称,编码器的名称	
