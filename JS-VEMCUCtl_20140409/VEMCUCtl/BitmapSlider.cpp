...
CBitmapSlider::CBitmapSlider()
{
	...
	m_LinePen1.CreatePenIndirect(&logPen);

	logPen.lopnColor = RGB(0,255,0);
	m_LinePen2.CreatePenIndirect(&logPen);
}
...
void CBitmapSlider::OnPaint() 
{
	...
	CMemDCN dc( &dcOrigin, &m_rect, m_bTransparentChannel );
	...
	COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	...
	//根据控件大小调整图片大小
	if(!ModifyBitmapSizeBySlider(m_bmChannel,nChannelID))
		return FALSE;
...
//		if( !m_bmChannelActive.LoadBitmap( nActiveID ) ) {
		if(!ModifyBitmapSizeBySlider(m_bmChannelActive,nActiveID)){//根据控件大小调整图片大小
...
void CBitmapSlider::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	::PostMessage(
		GetParent()->GetSafeHwnd(), WM_BITMAPSLIDER_LBUTTONDOWN,
		GetDlgCtrlID(), m_nPos );
...
BOOL CBitmapSlider::ModifyBitmapSizeBySlider(CBitmap &Bmp,UINT nBitmapID)
{
	RECT rc;
	CDC *pDC = NULL;
	CDC MemDC;
	...
	try
	{
		do 
		{
			...
			bResult = BmpTemp.LoadBitmap(nBitmapID);
			if(bResult == FALSE)
				return FALSE;

			if(BmpTemp.GetBitmap(&bm) == 0)
				return FALSE;
			...
			bResult = Bmp.CreateCompatibleBitmap(pDC,rc.right-rc.left,rc.bottom-rc.top);
			if (bResult == FALSE)
			{
				ReleaseDC(pDC);
				pDC = NULL;
			...
			bResult = MemDC2.CreateCompatibleDC(pDC);
			if (bResult == FALSE)
				break;
			...
			pOldBmp2 = MemDC2.SelectObject(&BmpTemp);
			if (pOldBmp2 == NULL)
			{
			...
			if (nWidth > 20&&bm.bmWidth > 20)
			{
				MemDC.BitBlt(0,0,20,bm.bmHeight,&MemDC2,0,0,SRCCOPY);
				MemDC.BitBlt(nWidth-20,0,20,bm.bmHeight,&MemDC2,bm.bmWidth-20,0,SRCCOPY);
				if (nWidth > 40&&bm.bmWidth > 40)
					MemDC.StretchBlt(20,0,nWidth-40,bm.bmHeight,&MemDC2,20,0,bm.bmWidth-40,bm.bmHeight,SRCCOPY);
				else
					MemDC.StretchBlt(20,0,nWidth-40,bm.bmHeight,&MemDC2,20,0,bm.bmWidth-20,bm.bmHeight,SRCCOPY);
			}
			else
			{
				MemDC.BitBlt(0,0,nWidth,bm.bmHeight,&MemDC2,0,0,SRCCOPY);
			}
...
BOOL CBitmapSlider::SetDrawLinePoint(int nPos)
{
	RECT rc;
	int nWidth = 0;
	...
	m_nLinePoint = nPos*nWidth/1000;

	return TRUE;
}

BOOL CBitmapSlider::SetDrawLinePointFlag(bool bFlag)
{
	m_bDrawLineFlag = bFlag;
	Invalidate(TRUE);
	return TRUE;
}

BOOL CBitmapSlider::DrawLineAroundPoint(CMemDCN &dc)
{
	CPen *pOldPen = NULL;

	pOldPen = dc.SelectObject(&m_LinePen1);
	dc.MoveTo(m_nMarginLeft,1);
	dc.LineTo(m_nLinePoint,1);

	dc.SelectObject(&m_LinePen2);
	dc.MoveTo(m_nLinePoint,1);
	dc.LineTo(m_nWidth-m_nMarginRight,1);
