...
#define HK_ZOOM_IN			11	/* 焦距以速度SS变大(倍率变大) */
#define HK_ZOOM_OUT		12	/* 焦距以速度SS变小(倍率变小) */
...
{
	DWORD dwSize;
	BYTE sDVRName[HK_NAME_LEN];     //DVR名称
...
typedef struct 
{
	...
	BYTE	sPPPoEUser[HK_NAME_LEN];							//PPPoE用户名
...
typedef struct 
{
	...
	BYTE sUsername[HK_NAME_LEN];		/* 用户名 */
...
{
	DWORD dwSize;
	BYTE sAlarmInName[HK_NAME_LEN];	/* 名称 */
...
typedef  struct tagNET_DVR_IPSERVER_STREAM
{
    ...
    BYTE    byDVRName[HK_NAME_LEN];    // DVR名称
    WORD    wDVRSerialLen;          // 序列号长度
    WORD    byRes1[2];              // 保留字节
    BYTE    byDVRSerialNumber[SERIALNO_LEN];    // DVR序列号长度
    BYTE    byUserName[HK_NAME_LEN];   // DVR 登陆用户名
...
typedef struct tagNET_DVR_DEV_CHAN_INFO
{
    ...
    BYTE	byRes[2];
    BYTE    byDomain[MAX_DOMAIN_NAME];	//设备域名
    BYTE	sUserName[HK_NAME_LEN];	//监控主机登陆帐号
...
typedef struct tagNET_DVR_DDNS_STREAM_CFG
{
    ...
    BYTE   sUserName[HK_NAME_LEN];   // DVR 登陆用户名
...
{
	DWORD dwSize;
	BYTE sAlarmOutName[HK_NAME_LEN];	/* 名称 */
...
typedef struct tagNET_DVR_ALARMHOST_LOG_RET
{
    ...
    BYTE		    sUserName[HK_NAME_LEN];     // 操作用户
...
typedef struct 
{
	...
	union
	{
		...
		{
			DWORD dwChannel;
			BYTE sUserName[HK_NAME_LEN];	/*请求视频用户名*/
			BYTE sPassword[PASSWD_LEN];	/* 密码 */
...
{		
	DWORD		dwSize;
	BYTE		sAccount[HK_NAME_LEN];				/* 账号*/ 
	BYTE		sPassword[MAX_EMAIL_PWD_LEN];			/*密码 */
	struct
	{
		BYTE	sName[HK_NAME_LEN];				/* 发件人姓名 */
...
typedef struct 
{
	BYTE sUsername[HK_NAME_LEN];  /* DDNS账号用户名/密码 */
...
typedef struct
{
	...
	BYTE sUsername[HK_NAME_LEN];			/* DDNS用户名*/
...
typedef struct
{
    ...
    struct
    {    
        BYTE sUsername[HK_NAME_LEN];			/* DDNS账号用户名*/
        BYTE sPassword[PASSWD_LEN];			/* 密码 */
        BYTE sDomainName[MAX_DOMAIN_NAME];	/* 设备配备的域名地址 */
...
{					
    ...
    BYTE	byRes2;						/* 保留 */
	TTY_CONFIG RemoteSerialDevCfg;
	BYTE  	byUsername[HK_NAME_LEN]; 		/* 32BYTES */
...
typedef struct tagNET_DVR_SUBSYSTEMINFO
{
    ...
    NET_DVR_IPADDR	struGatewayIpAddr;	/* 网关地址*/   
    BYTE		sUserName[HK_NAME_LEN];	/* 用户名 （此参数只能获取）*/
...
{   
    ...
    char sDeviceName[HK_NAME_LEN];		    /* 设备名字 */
		...
		struct 
		{
			...
			union
            {
				struct 
				{
                    ...
                    BYTE byRes1[2];
                    BYTE byAnonyIdentity [HK_NAME_LEN]; //匿名身份
					BYTE byUserName[HK_NAME_LEN]; //用户名
                    BYTE byPassword[HK_NAME_LEN]; //密码
                    BYTE byRes[44];
				...
				struct 
				{
                    BYTE byEapolVersion; //EAPOL版本，0-版本1，1-版本2
                    BYTE byRes1[3]; 
                    BYTE byIdentity[HK_NAME_LEN]; //身份
					BYTE byPrivateKeyPswd[HK_NAME_LEN]; //私钥密码
                    BYTE byRes[76]; 
				}EAP_TLS; 
            }auth_param;
...
typedef struct tagNET_VCA_ONE_RULE
{
    ...
    BYTE   byRuleName[HK_NAME_LEN];		//规则名称
...
typedef struct tagNET_DVR_SEARCH_EVENT_RET
{
	...
	union		
	{
		...
		struct //流id录像查询结果
		{
			...
			BYTE  byRes1[2];
			BYTE  byFileName[HK_NAME_LEN]; 	//文件名
			BYTE  byRes[260];
		}struStreamIDRet;
	}uSeniorRet;
...
typedef struct    tagNET_DVR_PTZ_POSITION
{
    ...
    BYTE byEnable;
    BYTE byRes1[3];  //保留
    BYTE byPtzPositionName[HK_NAME_LEN]; //场景位置名称
...
typedef struct tagNET_DVR_ONE_LANE
{
	...
	BYTE	byRes1[11];	// 保留字节
	BYTE    byLaneName[HK_NAME_LEN];       // 车道规则名称
...
{
    BYTE    byRuleName[HK_NAME_LEN];                 //车道规则名称 
...
typedef struct tagNET_DVR_SNMPCFG
{
    ...
    BYTE    byReadCommunity[HK_NAME_LEN]; //读共同体，最多31,默认"public"
    BYTE    byWriteCommunity[HK_NAME_LEN];//写共同体,最多31 字节,默认 "private"
...
typedef struct  tagNET_DVR_CODESPLITTERASSOCIATE
{
	...
	WORD		wPort;        //编码器端口号
	BYTE		byRes1[6];
    BYTE		sUserName[HK_NAME_LEN];	/* 用户名（32） */
...
{
	char sDeviceID[HK_NAME_LEN];//设备ID串
...
{
    BYTE byDeviceID[HK_NAME_LEN];/*设备的ID串*/
...
{
	BYTE byDialNum[HK_NAME_LEN];		/* dial number, eg. "#777" */
	BYTE byUserName[HK_NAME_LEN];	/* user name, eg. "card" */
	BYTE byPassword[DIALPASSWD_LEN];	/* password, eg. "card" */
	BYTE byApn[HK_NAME_LEN];		/* access point name, eg. "cmnet" */
...
{
	DWORD   dwSize;                 // 结构体大小
	BYTE    sDeviceName[HK_NAME_LEN];  // 前端设备名称 
...
typedef struct tagNET_DVR_SENSOR_ALARM
{
    ...
    BYTE    byName[HK_NAME_LEN]; 	// sensor 名称
...
{
    DWORD dwSize;
    BYTE   byName[HK_NAME_LEN]; 	// switch 名称
...
{
	DWORD 		dwSize;
	BYTE		sLEDName[HK_NAME_LEN/*32*/];	// LED屏幕名称
...
typedef struct tagNET_DVR_DISP_CHAN_INFO
{
    ...
    BYTE	bySlotNum;//槽位号
    BYTE	byRes[7];
    BYTE	sUserName[HK_NAME_LEN];	/*登陆帐号 */
    BYTE	sPassword[PASSWD_LEN];	/*密码 */
...
typedef struct tagNET_DVR_SOCKS_CFG
{
    ...
    BYTE 			byPassword[HK_NAME_LEN];			// 密码SOCKS5才用
...
typedef struct tagNET_DVR_CERT_INFO
{
	...
	BYTE     			byPasswd[HK_NAME_LEN];   //私钥密码
	...
	BYTE     			serialNumber[HK_NAME_LEN];   //证书标识码（自签名证书信息获取时有效）
	BYTE 				byVersion; 
...
{  
    ...
    BYTE  sCamName[HK_NAME_LEN]; /*cam的名称*/
    ...
    BYTE  sPurpose[HK_NAME_LEN];/*用途描述*/
	BYTE  byRes2[20];  
...
typedef struct tagNET_MATRIX_MATRIXINFO 
{
	...
	BYTE	sUserName[HK_NAME_LEN];   /*登录用户名*/
...
typedef struct tagNET_VCA_HUMAN_ATTRIBUTE
{
	...
	BYTE   byName[HK_NAME_LEN]; //姓名
	NET_DVR_AREAINFOCFG struNativePlace; //籍贯参数
	BYTE   byCertificateNumber[HK_NAME_LEN];  //证件号
	BYTE   byRes2[20];
...
typedef struct tagNET_VCA_BLACKLIST_INFO
{
	...
	BYTE   byRemark[HK_NAME_LEN]; //备注信息
	BYTE   byRes2[20];
...
typedef struct tagNET_DVR_DEV_ACCESS_CFG
{
	...
	BYTE 	byRes1;				//保留
    BYTE	sUserName[HK_NAME_LEN];	//接入设备的登录帐号
...
typedef struct tagNET_DVR_LAYOUTCFG
{
    ...
    BYTE  byValid;								//布局是否有效
    BYTE  byRes1[3];
    BYTE  byLayoutName[HK_NAME_LEN/*32*/];			//布局名称			
...
typedef struct tagNET_DVR_INPUTSTREAMCFG
{
    ...
    BYTE	sCamName[HK_NAME_LEN] ;			//信号输入源名称
    ...
    BYTE	sGroupName[HK_NAME_LEN/*32*/];	//网络信号源分组 组名
...
typedef struct tagNET_DVR_SCREEN_SCREENINFO
{
    ...
    BYTE sUserName[HK_NAME_LEN];	/*登录用户名*/
    BYTE sPassword[PASSWD_LEN]; /*登录密码*/   
	BYTE sDevName[HK_NAME_LEN];	/*设备名称*/
...
typedef struct tagNET_DVR_PLAN_CFG
{
	...
	BYTE	byRes1[2];
	BYTE  	byPlanName[HK_NAME_LEN/*32*/]; //预案名称
...
{
	DWORD	dwSize;
	BYTE	sDeviceID[HK_NAME_LEN]; //被控设备的设备ID
	WORD	wChan;				 //被控通道
...
typedef struct tagNET_DVR_ALARMHOST_WIRELESS_NETWORK_CFG
{
	...
	BYTE	byRes1;
	BYTE	bySIMNum[HK_NAME_LEN/*32*/]; //SIM卡号（手机号）
...
typedef struct tagNET_DVR_CID_ALARM
{
    ...
    BYTE	sCIDDescribe[HK_NAME_LEN/*32*/];	//CID事件名
	...
	BYTE	sUserName[HK_NAME_LEN/*32*/];		//网络用户用户名
...
typedef struct tagNET_VCA_DATABASE_PARAM
{
	...
	BYTE byDataBaseName[HK_NAME_LEN]; //数据库名称
...
typedef struct tagNET_VCA_INQUIRE_SNAPDB_COND
{
	...
	BYTE byStartBirthDate[MAX_HUMAN_BIRTHDATE_LEN]; //起始出生年月，如：201106
	BYTE byEndBirthDate[MAX_HUMAN_BIRTHDATE_LEN]; //截止出生年月，如201106
	BYTE byAttribute1[HK_NAME_LEN]; //属性1
	BYTE byAttribute2[HK_NAME_LEN]; //属性2
	BYTE byRes[12]; //保留
...
{ 
	...
	BYTE  byRemark1[HK_NAME_LEN]; //备注信息1
...
{
    BYTE byAlarmName[HK_NAME_LEN];	/* 报警名称 */
...
typedef struct tagNET_DVR_DECODER_JOINT_PARAM
{
	...
	BYTE	sDeviceName[HK_NAME_LEN];	//被关联解码器的设备名称
	BYTE	sChanName[HK_NAME_LEN];	//被关联解码器的解码通道名称
	BYTE	byRes2[32];		// 保留字节
...
typedef struct tagNET_DVR_CHAN_INFO
{
	...
	BYTE   byRes1[2];
	BYTE   sChanName[HK_NAME_LEN];    /*通道名称*/
...
{
	DWORD	dwSize;
	BYTE	byDevName[HK_NAME_LEN/*32*/];//网络信号源名称
	...
	BYTE 	byRes1[3];	//保留
    BYTE	sGroupName[HK_NAME_LEN/*32*/];//网络信号源所在组的组名，网络信号源根据这个组名来分类
    BYTE	byRes2[28];	//保留		
...
{
	BYTE				byFileName[HK_NAME_LEN]; 	// 录像文件名，老协议只传32个文件名
    DWORD				dwLockDuration;			// 锁定持续时间,单位秒，0xffffffff表示永久锁定
	BYTE				byRes[512];
...
{
	DWORD dwSize;
	BYTE  sSceneName[HK_NAME_LEN];//场景名称
	BYTE  byRes [80];
...
typedef struct tagNET_DVR_EXTERNAL_MATRIX_CFG
{
	...
	BYTE		byRes1[3];
	BYTE	    sMatrixName[HK_NAME_LEN/*32*/];       // 矩阵的名称
...
{
	DWORD   dwSize;
	BYTE	sUserName[HK_NAME_LEN];			// 用户名，最大16字节
	BYTE	sPassWord[PASSWD_LEN];		// 密码
	NET_DVR_IPADDR	struUserIP;	// 用户IP地址(为0时表示允许任何地址)
	BYTE	byMacAddr[MACADDR_LEN];			// 物理地址
